structure PrintAbsyn : sig val print : TextIO.outstream * Absyn.exp -> unit end =
struct
  ...
  fun print (outstream, e0) =
    let fun say s =  TextIO.output(outstream, s)
        ...
        and exp(A.VarExp v, d) =
          ...
          | exp(A.SeqExp l, d) =
              (indent d; say "SeqExp["; dolist d exp (map #1 l);  say "]")
          ...
          | exp(A.LetExp{decs,body,pos},d) =
		          (indent d; say "LetExp(["; dolist d dec decs;
        ...
        and dec(A.FunctionDec l, d) =
          ...
          | dec(A.TypeDec l, d) =
              let fun tdec({name,ty=t,pos},d) = (indent d; say"(";
                say(Symbol.name name); sayln ","; ty(t,d+1); say ")")
